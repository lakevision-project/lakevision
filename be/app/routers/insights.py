...
from app.dependencies import lv
...
def get_latest_table_insights(table_id: str, size: int = Query(5, ge=1)):
...
def get_table_insights(table_id: str, rule_ids: Optional[List[str]] = Query(None, alias="rule_ids")):
    try:
...
def get_namespace_insights(namespace: str, recursive: bool = Query(True)):
    try:
